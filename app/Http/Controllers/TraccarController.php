...
class TraccarController extends Controller
{
    ...
    public function devices($id){
        if(TripMaster::where('device_id', $id)->where('grade', 0)->count() > 0){
            $lesson = TripMaster::where('device_id', $id)->first();
    ...
    public function getMaxSpeedByLesson($lesson_id){
        ...
        $max_speed = TcPositions::where('deviceid', $lesson->device_id)
                                     ->whereBetween('devicetime', [$lesson->end_time, $lesson->end_time])
                                     ->orderBy('speed', 'DESC')->first();
        if($max_speed && $max_speed->count()){
            $lesson->speed = round($max_speed->speed*1.85, 2);
            $lesson->save();
    ...
    //     $lesson = TripMaster::where('id', $lesson_id)->first();
    //     $positions = TcPositions::where('deviceid', $lesson->device_id)->whereBetween('devicetime', [$lesson->end_time, $lesson->end_time])->get();
    //     $previous_item = null;
    //     $brakes_counter = 0;
    ...
    public function getFullDistance($lesson_id){
        $lesson = TripMaster::where('id', $lesson_id)->first();
        $attributes = TcPositions::where('deviceid', $lesson->device_id)->whereBetween('devicetime', [$lesson->start_time, $lesson->end_time])->pluck('attributes');
        $total_distance = 0;
    ...
    public function getFullDistanceByStudent($student_id){
        $TripMaster = TripMaster::where('student_id', $student_id)->get();
        $attributes_arr = [];
        foreach($TripMaster as $lesson){
            $attributes_arr[] = TcPositions::where('deviceid', $lesson->device_id)->whereBetween('devicetime', [$lesson->start_time, $lesson->end_time])->pluck('attributes');
        }
        $total_distance = 0;
    ...
    public function getLessonPositions($lesson_id){
        $lesson = TripMaster::where('id', $lesson_id)->first();
        return TcPositions::where('deviceid', $lesson->device_id)->whereBetween('devicetime', [$lesson->start_time, $lesson->end_time])->get(['latitude', 'longitude']);
    }
